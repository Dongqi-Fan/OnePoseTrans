...
source_imgs = []
target_imgs = []
with open('files/wpose_test_pairs.txt', 'r') as f:
    all_lines = f.readlines()
for line in all_lines:
    line = line.rstrip()
    name1, name2 = line.split(' ')[0], line.split(' ')[1]
    source_imgs.append(name1)
    target_imgs.append(name2)


# get discrete descriptions
negative_prompt = "low quality, bad quality"
with open('files/wpose_descriptions.txt', 'r') as f:
    all_lines = f.readlines()
descriptions = dict()
for line in all_lines:
    line = line.rstrip()
...
for source_img, target_img in zip(source_imgs, target_imgs):
    source_rgba_img = Image.open(rgba_path + source_img).resize((1024, 1024))
    # pose_image = Image.open(pose_path + target_img).resize((1024, 1024))
    pose_image = build_pose_img_openpose(real_path + target_img)

    prompt = descriptions[source_img]
    lora = lora_path + source_img.replace('png', 'safetensors')
    ...
    # saving
    Image.fromarray(result_list[0]).save('/your/path/to/save/results/reposed_result/{}_to_{}.png'.format(
